...
# ---------------------------------------------------------
# Moroccan Account Financial Report
# ---------------------------------------------------------

class account_financial_report_ma(osv.osv):
    ...
    def _get_account_by_class(self, cr, uid, ids, context=None):
        res = []
	obj_acc=self.pool.get('account.account')
	for cls in obj_acc.browse(cr, uid, ids, context=context):
        	if cls.parent_id.code == 'CLASSE_1' and cls.level == 2:
       			res[cls.id] = cls
        return res
    ...
    #'code': fields.char('code du tableau (liasse fiscale)', required=False, translate=True),
	     #}

    #def _get_children_by_class(self, cr, uid, ids, context=None):
        #'''returns a dictionary with the key= the ID of a record and value = all its children,
         #  computed recursively, and sorted by sequence. Ready for the printing'''
        #res = []
        #for id in ids:
         #   res.append(id)
          #  ids2 = self.search(cr, uid, [('parent_id.code', '=', 'CLASSE_1'),'|',('parent_id', '=', id),'|',('level','=',2)], order='sequence ASC', context=context)
           # res += self._get_children_by_order(cr, uid, ids2, context=context)
        #return res
